...
plot_pffr <- function(m) {
  layout(t(1:3))
  plot(m, select = 1, shift = m$coefficients[1], scheme = 1) # ! shift by intercept
...
```{r}
save.image(file = "gfamm-checks-sim.RData")
```
...
df_long <- readRDS(here::here("data/df_long.rds")) |> 
  filter(as.numeric(id) < 10 & (index %% 10 == 0))
...
logLik(pffr_model_pc)
logLik(pffr_model_re)
dbinom(x = df_pffr$Y, size = 1, prob = plogis(nhanes_gcfpca$etas), log = T) |> sum()
logLik(nhanes_gcfpca$model)
```
...
#! shouldn't the intercept functions be shifted by the constant Intercept?
...
plot_df %>%
  ...
  geom_line(aes(color = "GCFPCA")) +
  geom_line(aes(x = sind, y = betahat_pffr, color = "GFAMM PC")) +
  scale_fill_manual(values = c("CI" = "blue", "CI Joint" = "lightblue", "CI pffr" = "pink"), name = "Confidence Interval") +
  scale_color_manual(values = c("GCFPCA" = "blue", "GFAMM PC" = "red"), name = "Confidence Interval") +
  # Adding a horizontal dotted line at y = 0
  geom_hline(yintercept = 0, linetype = "dotted") +
  # Setting x-axis labels to show time
  scale_x_continuous(breaks = seq(0, 1, by = 0.2)) +
...
plot_df %>%
  ...
  scale_fill_manual(values = c("CI" = "blue", "CI Joint" = "lightblue", "CI pffr" = "pink"), name = "Confidence Interval") +
  scale_color_manual(values = c("GCFPCA" = "blue", "GFAMM RE" = "red"), name = "Confidence Interval") +
  # Adding a horizontal dotted line at y = 0
  geom_hline(yintercept = 0, linetype = "dotted") +
  # Setting x-axis labels to show time
  scale_x_continuous(breaks = seq(0, 1, by = 0.2)) +
...
```{r}
save.image(file = "gfamm-checks-full.RData")
```
